	...
	public class Textension: MonoBehaviour
	{
		...
		private void Initialize()
		{
			...
			for (int i = 0; i < text.text.Length; i++)
			{
                ...
                characters.Add(c);
                c.cs.scale = characterScale * Vector3.one;
                
                // TODO: Don't add effects to spaces / some characters
		...
		private void Update()
		{
            ...
            // Step 2: Calculate the character effect
            EffectTick();
        ...
        private void EffectTick()
        {
            ...
            for (int i = 0; i < GetCharacters().Count; i++)
            {
                ...
                for (int j = 0; j < character.GetEffectsCount(); j++)
                { 
                    // Set scale to that individual character using it's own data
                    character.SetScale(character.GetEffect(j).Calculate(character) * Vector3.one);
                }
            }
        }
